...
echo "
		...
		";
		$sqls="SELECT application.a_type,app_info.i_aid,application.a_given_name,application.a_last_name,app_info.app_mobile,app_info.app_email FROM app_info INNER JOIN application INNER JOIN app_stage ON app_info.i_aid = application.aid and app_stage.s_aid=app_info.i_aid WHERE app_info.a_stage = 2 ";
		...
		if(mysqli_num_rows($resv) > 0)
		{
			...
			while($rowv = mysqli_fetch_array($resv)) {
				...
				echo "<td><font size=4><a href='application_verification.php?q=".$rowv['i_aid']."'>$a_id</a></td>";
			...
			echo "<p>No Applications to verify.</p>";}
		echo "</table>";
		?>
	...
	<div class="w3-container w3-card w3-white w3-round w3-margin">
		...
		<table class='w3-table-all'>
		";
		$sqls="SELECT app_info.i_aid,application.a_given_name,application.a_last_name,app_info.app_mobile,app_info.app_email , application.a_type FROM app_info INNER JOIN application INNER JOIN app_stage ON app_info.i_aid = application.aid and app_stage.s_aid=app_info.i_aid WHERE app_info.a_stage = 6 ";
		$resv = mysqli_query($link,$sqls);
		if(mysqli_num_rows($resv) > 0)
		{
			echo "
			...
			while($rowv = mysqli_fetch_array($resv)) {
    			...
    			echo "<tr>";
				$a_id=$rowv['a_type'].'_'.$rowv['i_aid'];
				echo "<td><font size=4><a href='final_verification.php?q=".$rowv['i_aid']."'>$a_id</a></td>";
     			echo "<td>" . $rowv['a_given_name'] . " " . $rowv['a_last_name'] . "</td>";
    			echo "<td>" . $rowv['app_mobile'] . "</td>";
    			echo "<td>" . $rowv['app_email'] . "</td>";
     			echo "</tr>";
